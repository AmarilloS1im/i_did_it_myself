'''
В файл Task_from_PVB.xlsx проставить нужные артикулы для поиска в первый столбец. Шапку с заголовками оставить. Всю остальную информацию удалить(если она есть)
...
def GetData(session,processed_list,headers):
    ...
    for articles in processed_list:
        ...
        if len(user_friendly_json["ProductLists"]) == 0 or user_friendly_json["ProductLists"][0]['Groups'][0]['Manufacturers'] == None :
